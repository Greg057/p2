...
function useOrbitingCircles(skillsWithLogos: Array<{name: string, logo: string}>, maxOrbitingTechs: number = 36) {
  // Limit to maximum 36 technologies (3 circles × 12 techs per circle)
  const orbitingTechs = skillsWithLogos.slice(0, maxOrbitingTechs);
  const remainingTechs = skillsWithLogos.slice(maxOrbitingTechs);

  // Determine circle configuration based on number of technologies (max 3 circles)
  const getCircleConfig = (techCount: number) => {
    if (techCount <= 8) {
      // Single circle - up to 8 techs
      return {
        circles: [{
          techs: orbitingTechs,
      ...
      // Two circles - distribute proportionally
      ...
      // Three circles - distribute proportionally (max 3 circles)
      ...
      // Too many techs - fall back to grid
      return { circles: [] };
    }
  };
...
export default function OrbitingCirclesStatic({ skills }: StaticSkillsData) {
  ...
  return (
    <section id="skills" className="mb-28">
      ...
      {/* Mobile: Show clean grid layout for all technologies with logos */}
      ...
      {/* Desktop: Show orbiting animation */}
      ...
      {/* Desktop: Grid layout for remaining technologies with logos (if any) */}
